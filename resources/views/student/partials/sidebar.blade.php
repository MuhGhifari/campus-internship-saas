@php
    $links = [
        ...
        ['offers.index', 'Cari Lowongan', 'offers.*'],
...
<aside class="bg-emerald-950 px-5 py-6 text-white">
    <a href="{{ route('dashboard') }}" class="flex items-center gap-3">
        <span class="grid h-11 w-11 place-items-center rounded-lg bg-emerald-400 font-bold text-emerald-950">CB</span>
        <span>
            <span class="block text-sm font-semibold uppercase tracking-wide text-emerald-200">Ruang Mahasiswa</span>
    ...
    <div class="mt-7 rounded-lg border border-white/10 bg-white/5 p-4">
        ...
        <p class="mt-1 text-xs text-white/60">Lowongan resmi dari kampus Anda</p>
    </div>

    <nav class="mt-7 space-y-1 text-sm">
        @foreach ($links as [$route, $label, $active])
            @php($isActive = request()->routeIs($active))
            <a class="block rounded-lg px-3 py-2 {{ $isActive ? 'bg-emerald-400 text-emerald-950 font-bold' : 'text-white/75 hover:bg-white/10 hover:text-white' }}" href="{{ route($route) }}">{{ $label }}</a>
        @endforeach
    </nav>
