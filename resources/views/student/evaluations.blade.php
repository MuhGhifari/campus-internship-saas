...
@section('content')
    <div class="mb-8">
        ...
        <p class="mt-3 text-[#6B7E94]">Penilaian akhir diisi oleh PJ perusahaan; pembimbing kampus dan staf hanya memantau hasilnya.</p>
    </div>
        ...
        @forelse ($applications as $application)
            <section class="cb-card p-6">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    ...
                    @if ($application->companyEvaluation)
                        <span class="rounded-full bg-[#FDF3DC] px-4 py-2 text-sm font-bold text-[#0D1B2A]">Rata-rata {{ $application->companyEvaluation->rataRata() }}</span>
                    @endif
                </div>

                @if (auth()->user()->hasRole('company_supervisor'))
                        ...
                        <form method="POST" action="{{ route('evaluations.store', $application) }}" class="grid gap-4 sm:grid-cols-4">
                            ...
                            @foreach ([
                                ...
                                <label class="block">
                                    ...
                                    <input name="{{ $field }}" type="number" min="0" max="100" value="{{ old($field, $application->companyEvaluation->{$field} ?? 80) }}" class="cb-input mt-2">
                                </label>
                            @endforeach
                            <label class="block sm:col-span-4">
                                <span class="text-sm font-medium">Catatan</span>
                                <textarea name="catatan" rows="3" class="cb-input mt-2">{{ old('catatan', $application->companyEvaluation->catatan ?? '') }}</textarea>
                            </label>
